...
function chapter(
  ...
  return { id, chapter: chapterNumber, patientBeat, learningObjective, sources, draft }
}

export const OFFICIAL_CASES: readonly OfficialCaseChapter[] = [
  chapter('case-chapter-1-wound', 1, '患者运动时发生皮肤擦伤，局部组织首先需要稳定氧供。', '理解红细胞运输氧气如何支持创伤修复。', {
    ...
    nodeCounts: { oxygenRoutes: 1, infectionSites: 0 }, allowedEvents: ['ACUTE_HYPOXIA'], educationalTopic: '创伤修复与氧供', stabilitySeconds: 4,
  }, [SOURCE_LIBRARY.oxygen]),
  chapter('case-chapter-2-invasion', 2, '擦伤处出现细菌入侵，患者局部红肿，白细胞开始防御。', '理解先天免疫如何识别并清除感染灶。', {
    ...
    nodeCounts: { oxygenRoutes: 0, infectionSites: 2 }, allowedEvents: ['INFECTION_REBOUND', 'ATP_CRISIS'], educationalTopic: '白细胞与先天免疫', stabilitySeconds: 5,
  }, [SOURCE_LIBRARY.immunity]),
  chapter('case-chapter-3-hypoxia', 3, '炎症增加了耗氧，患者出现短暂气促，需要恢复肺泡到组织的运输。', '理解炎症期氧需求上升与红细胞职责。', {
    ...
    nodeCounts: { oxygenRoutes: 2, infectionSites: 0 }, allowedEvents: ['ACUTE_HYPOXIA', 'TRANSPORT_BLOCKAGE'], educationalTopic: '氧合与循环运输', stabilitySeconds: 5,
  }, [SOURCE_LIBRARY.oxygen, SOURCE_LIBRARY.inflammation]),
  chapter('case-chapter-4-rebound', 4, '感染一度反弹，患者发热，白细胞需要控制扩散而不是追求击杀分数。', '理解感染控制以患者指标稳定为结果。', {
    ...
    nodeCounts: { oxygenRoutes: 0, infectionSites: 3 }, allowedEvents: ['INFECTION_REBOUND', 'ATP_CRISIS'], educationalTopic: '感染反弹与炎症控制', stabilitySeconds: 6,
  }, [SOURCE_LIBRARY.immunity, SOURCE_LIBRARY.inflammation]),
  chapter('case-chapter-5-cooperation', 5, '患者进入恢复期，运输与防御必须同时工作，任何单一职责都不足以稳定指标。', '理解红白细胞协作维持内环境稳定。', {
    ...
    nodeCounts: { oxygenRoutes: 1, infectionSites: 1 }, allowedEvents: ['ACUTE_HYPOXIA', 'INFECTION_REBOUND', 'TRANSPORT_BLOCKAGE'], educationalTopic: '运输、防御与组织修复', stabilitySeconds: 6,
  }, [SOURCE_LIBRARY.oxygen, SOURCE_LIBRARY.immunity]),
  chapter('case-chapter-6-recovery', 6, '患者完成康复，免疫记忆形成，最终报告回顾运输、防御与恢复的协作。', '综合解释氧运输、免疫防御和免疫记忆。', {
    ...
    nodeCounts: { oxygenRoutes: 2, infectionSites: 2 }, allowedEvents: ['ACUTE_HYPOXIA', 'INFECTION_REBOUND', 'TRANSPORT_BLOCKAGE', 'ATP_CRISIS'], educationalTopic: '适应性免疫与免疫记忆', stabilitySeconds: 7,
